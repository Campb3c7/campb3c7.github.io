addQuestions([
  {
    ...
    choices: ["Alpha Thalassemia Major (Hb Bart's/hydrops fetalis) — each parent can pass a chromosome with both alpha genes deleted, resulting in a fetus with all 4 alpha genes deleted and no functional alpha-chain production", "Beta Thalassemia Major — the parents' genotypes are irrelevant to alpha-globin genes", "Isolated iron deficiency anemia unrelated to genetics", "Hemoglobin H disease, which does not cause hydrops"],
    correct: 0,
  ...
  },
  {
    ...
    choices: ["HbH (β4), which only forms after birth and is irrelevant to fetal physiology", "HbS, a sickle variant unrelated to thalassemia", "HbA (α2β2), which is unaffected by alpha gene deletions", "Hemoglobin Bart's (γ4) — it has an abnormally high oxygen affinity, so it picks up oxygen but does not release it effectively to peripheral tissues"],
    correct: 3,
  ...
  },
  {
    ...
    choices: ["This distinction only matters for beta-thalassemia risk", "A parent with a cis deletion can pass an entire chromosome with zero functional alpha genes to a child, raising the risk of Alpha Thalassemia Major if the partner also carries a cis deletion — a risk that trans-configuration carriers do not pass on in the same way", "It doesn't matter; both configurations carry identical risk of Alpha Thalassemia Major in offspring", "Trans-configuration carriers are at higher risk of passing Alpha Thalassemia Major"],
    correct: 1,
